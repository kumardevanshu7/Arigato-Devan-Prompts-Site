...
$id = (int)($_GET['id'] ?? 0);
$slug = trim($_GET['slug'] ?? '');
if ($id <= 0 && $slug === '') { header('Location: not_mine.php'); exit(); }
if (!nm_table_exists_boot($pdo, 'not_mine_prompts')) { header('Location: not_mine.php'); exit(); }

if ($slug !== '') {
    $stmt = $pdo->prepare('SELECT * FROM not_mine_prompts WHERE slug = ? AND is_visible = 1');
    $stmt->execute([$slug]);
} else {
    $stmt = $pdo->prepare('SELECT * FROM not_mine_prompts WHERE id = ? AND is_visible = 1');
    $stmt->execute([$id]);
}
$p = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$p) { header('Location: not_mine.php'); exit(); }
$id = (int) $p['id'];

$is_legacy_route = basename($_SERVER['SCRIPT_FILENAME'] ?? '') === 'not_mine_prompt.php';
...
$user_vote = null;
if ($user_id) {
    $vs = $pdo->prepare('SELECT voted_for FROM not_mine_votes WHERE user_id = ? AND prompt_id = ?');
...
{
    try {
        $st = $pdo->prepare('SELECT COUNT(*) FROM not_mine_likes WHERE prompt_id = ?');
...
{
    try {
        $pdo->prepare("INSERT IGNORE INTO not_mine_likes (user_id, prompt_id, like_type) VALUES (?, ?, 'unlock')")
...
$vc = $pdo->prepare("SELECT voted_for, COUNT(*) as cnt FROM not_mine_votes WHERE prompt_id = ? GROUP BY voted_for");
$vc->execute([$id]);
...
if ($user_id) {
    try {
        if (nm_has_column($pdo, 'not_mine_likes', 'like_type')) {
            $lk = $pdo->prepare("SELECT id FROM not_mine_likes WHERE user_id = ? AND prompt_id = ? AND like_type = 'manual'");
            $lk->execute([$user_id, $id]);
        } else {
            $lk = $pdo->prepare("SELECT id FROM not_mine_likes WHERE user_id = ? AND prompt_id = ?");
            $lk->execute([$user_id, $id]);
        }
        $is_liked = !!$lk->fetch();
    } catch (PDOException $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['ajax'] ?? '') === 'vote') {
    ...
    try {
        $pdo->prepare('INSERT INTO not_mine_votes (user_id, prompt_id, voted_for) VALUES (?, ?, ?)')->execute([$user_id, $id, $vote]);
        nm_unlock_like($pdo, $user_id, $id);
    } catch (PDOException $e) {
        echo json_encode(['ok' => false, 'msg' => 'Already voted']); exit();
    }
    $vc2 = $pdo->prepare("SELECT voted_for, COUNT(*) as cnt FROM not_mine_votes WHERE prompt_id = ? GROUP BY voted_for");
    $vc2->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['ajax'] ?? '') === 'like') {
    ...
    try {
        if (nm_has_column($pdo, 'not_mine_likes', 'like_type')) {
            $ex = $pdo->prepare("SELECT id FROM not_mine_likes WHERE user_id = ? AND prompt_id = ? AND like_type = 'manual'");
            $ex->execute([$user_id, $id]);
            if ($ex->fetch()) {
                $pdo->prepare("DELETE FROM not_mine_likes WHERE user_id = ? AND prompt_id = ? AND like_type = 'manual'")->execute([$user_id, $id]);
                $liked = false;
            } else {
                $pdo->prepare("INSERT INTO not_mine_likes (user_id, prompt_id, like_type) VALUES (?, ?, 'manual')")->execute([$user_id, $id]);
                $liked = true;
            }
        } else {
            $ex = $pdo->prepare("SELECT id FROM not_mine_likes WHERE user_id = ? AND prompt_id = ?");
            $ex->execute([$user_id, $id]);
            if ($ex->fetch()) {
                $pdo->prepare("DELETE FROM not_mine_likes WHERE user_id = ? AND prompt_id = ?")->execute([$user_id, $id]);
                $liked = false;
            } else {
                $pdo->prepare("INSERT INTO not_mine_likes (user_id, prompt_id) VALUES (?, ?)")->execute([$user_id, $id]);
                $liked = true;
            }
        }
    } catch (PDOException $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['ajax'] ?? '') === 'save') {
    ...
    try {
        if (nm_has_column($pdo, 'saved_prompts', 'source')) {
            $already = $pdo->prepare("SELECT id FROM saved_prompts WHERE user_id = ? AND prompt_id = ? AND source = 'not_mine'");
            $already->execute([$user_id, $id]);
            if ($already->fetch()) {
                $pdo->prepare("DELETE FROM saved_prompts WHERE user_id = ? AND prompt_id = ? AND source = 'not_mine'")->execute([$user_id, $id]);
                echo json_encode(['ok' => true, 'saved' => false]);
            } else {
                $pdo->prepare("INSERT INTO saved_prompts (user_id, prompt_id, source) VALUES (?, ?, 'not_mine')")->execute([$user_id, $id]);
...
if ($user_id) {
    try {
        if (nm_has_column($pdo, 'saved_prompts', 'source')) {
            $sv = $pdo->prepare("SELECT id FROM saved_prompts WHERE user_id = ? AND prompt_id = ? AND source = 'not_mine'");
...
if ($meta_desc === '') {
    $meta_desc = htmlspecialchars($p['title']) . ' — ChatGPT vs Gemini AI prompt battle on Not Mine | Arigato Devan';
} else {
...
<head>
    ...
    <base href="<?= $base_path ?>">
    <title><?= htmlspecialchars($p['title']) ?> — Not Mine | Arigato Devan</title>
    <meta name="description" content="<?= $meta_desc ?>">
    <?php if ($meta_keywords !== ''): ?><meta name="keywords" content="<?= htmlspecialchars($meta_keywords) ?>"><?php endif; ?>
    <meta property="og:type" content="article">
    <meta property="og:title" content="<?= htmlspecialchars($p['title']) ?> — Not Mine | Arigato Devan">
...
<div class="nmp">
    <a href="not_mine.php?cat=<?= $p['category'] ?>" class="nmp-back"><i class="fa-solid fa-arrow-left"></i> Back to Not Mine</a>
