...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$title) {
        ...
        if (empty($error)) {
            ...
            if ($has_fk_col) {
                $stmt = $pdo->prepare(
                    "INSERT INTO blogs (title, slug, description, content, content_hindi, image_path, image_path_landscape, image_ratio, meta_title, meta_description, tags, focus_keyword, category, is_published, author_id, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())"
                );
                $stmt->execute([
                    $title,
                    $slug,
...
// Focus Keyword draft persistence with localStorage
(function() {
    const fkInput = document.getElementById('seo-focus-keyword');
    if (!fkInput) return;
    const storageKey = 'blog_create_focus_kw';
...
<!-- Table Template Designer Modal -->
<div id="tableTemplateModal" class="be-modal-backdrop" style="display:none; position:fixed; inset:0; z-index:99999; background:rgba(15,23,42,0.65); backdrop-filter:blur(5px); align-items:center; justify-content:center; padding:16px;">
    <div class="be-modal-dialog" style="background:#ffffff; border-radius:20px; max-width:740px; width:100%; max-height:92vh; overflow-y:auto; box-shadow:0 25px 50px -12px rgba(0,0,0,0.3); border:1px solid #e2e8f0; font-family:'Plus Jakarta Sans',sans-serif;">
        <!-- Modal Header -->
        <div style="padding:18px 24px; border-bottom:1px solid #f1f5f9; display:flex; align-items:center; justify-content:space-between;">
            <div style="display:flex; align-items:center; gap:12px;">
