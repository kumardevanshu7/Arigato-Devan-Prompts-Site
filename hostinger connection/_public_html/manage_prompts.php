...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'admin') {
    $_SESSION['error_msg'] = "You do not have permission to access the manage prompts page.";
...
<head>
    ...
    <style>
        ...
        .prompt-item.is-unreleased { background: #fffaf0; border-color: #ffd8a8; }
        .prompt-item.is-unreleased:hover { border-color: #f59f00; }
        ...
        .unreleased-badge { background: #fff3bf; color: #d9480f; font-size: 0.65rem; padding: 2px 8px; border-radius: 12px; font-weight: 900; border: 1px solid #ffc078; text-transform: uppercase; }
    ...
    <div class="dashboard-wrap">
        ...
        <div class="dash-card">
            ...
            foreach($prompts as $p) {
                $tarr = explode(',', $p['tag']);
                foreach($tarr as $t) { $t = trim(strtolower($t)); if(!empty($t)) $all_mgr_tags[] = $t; }
            }
            ...
            <?php if(count($prompts)===0): ?>
                <p style="text-align:center;color:#7D7887;font-weight:600;padding:60px 0;font-size:1.1rem;">No prompts yet — start uploading!</p>
            <?php else: ?>
            <div id="prompts-list">
                <?php foreach($prompts as $p):
                    $p_tags_arr = array_map('trim', explode(',', strtolower($p['tag'])));
                    $ptype = 'secret';
                    if (in_array('viral', $p_tags_arr)) $ptype = 'viral';
                    elseif (in_array('unreleased', $p_tags_arr)) $ptype = 'unreleased';
                    $type_badge_map = ['secret'=>'SCP','unreleased'=>'URP','viral'=>'IVP'];
                    $type_badge_cls = ['secret'=>'scp','unreleased'=>'urp','viral'=>'ivp'];
                ?>
                <div class="prompt-item" data-title="<?=strtolower(htmlspecialchars($p['title']))?>" data-tags="<?=htmlspecialchars(implode(',', $p_tags_arr))?>">
                    <img src="<?=htmlspecialchars($p['image_path'])?>" class="prompt-item-img" alt="Cover">
                    <div class="prompt-item-details">
                        ...
                        <div class="prompt-item-meta">
                            <?php if($ptype === 'secret'): ?>
                                Code: <span class="code-badge"><?=htmlspecialchars($p['unlock_code'])?></span> &nbsp;|&nbsp;
                            <?php endif; ?>
                            <i class="fa-solid fa-heart"></i> <?=(int)$p['likes_count']?>
                            &nbsp;|&nbsp;
                            <?php foreach($p_tags_arr as $t): if(empty($t)) continue; ?>
                                <span style="background:var(--secondary-color);padding:2px 7px;border-radius:10px;font-size:0.75rem;font-weight:800;border:1.5px solid var(--text-color);">#<?=htmlspecialchars($t)?></span>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <div style="display:flex;gap:8px;flex-shrink:0;">
                        <a href="edit_prompt.php?id=<?=$p['id']?>" class="edit-btn" title="Edit"><i class="fa-solid fa-pencil"></i></a>
                        <button class="delete-btn" title="Delete" onclick="confirmDelete(<?=$p['id']?>, '<?=addslashes(htmlspecialchars($p['title']))?>')">
