...
<header>
  ...
  <nav class="nav-links"><a href="digital_store/index.php" class="shop-nav-btn"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg> SHOP</a><a href="dashboard.php">DASHBOARD</a><a href="blog_admin.php" style="background:var(--primary-color);border:2px solid var(--text-color);box-shadow:3px 3px 0 var(--text-color);border-radius:20px;"><i class="fa-solid fa-pencil"></i> BLOGS</a></nav>
...
<?php if ($edit_error): ?>
  <div class="bc-wrap" style="padding: 20px 20px 0;"><div class="flash-error"><i class="fa-solid fa-triangle-exclamation"></i> <?= htmlspecialchars($edit_error) ?></div></div>
<?php endif; ?>
...
<form method="POST" action="blog_edit.php?id=<?= $id ?>" enctype="multipart/form-data" class="scribe-container">
  ...
  <aside class="scribe-sidebar">
    <div>
      ...
      <div class="sidebar-links">
        ...
        <a href="blog_create.php" class="sidebar-link"><i class="fa-solid fa-plus"></i> Create Post</a>
    ...
    <div>
      <div class="sidebar-section-title">Editing Mode</div>
      <div style="font-size: 0.8rem; font-weight: 600; color: #8c8994; line-height: 1.5; padding: 0 10px;">
        You are currently editing: <strong style="color: var(--text-color);"><?= htmlspecialchars($bl["title"]) ?></strong>. Your edits won't be visible to readers until you hit Save and publish.
      </div>
    </div>
  </aside>
  ...
  <main class="scribe-workspace">
    <div class="scribe-paper">
      ...
      <input type="text" class="scribe-title-input" id="bc-title" name="title" value="<?= htmlspecialchars($bl["title"]) ?>" placeholder="Your amazing blog title..." required autocomplete="off">
      
      <!-- Short Description/Summary (borderless) -->
      <textarea class="scribe-desc-input" id="bc-desc" name="description" placeholder="Write a short summary shown on blog list cards..." rows="1" oninput="this.style.height = ''; this.style.height = this.scrollHeight + 'px'"><?= htmlspecialchars($bl["description"] ?? "") ?></textarea>
    ...
    <div class="control-card" style="border-color: var(--text-color); background: #faf9ff;">
      ...
      <div class="fg" style="margin-bottom: 12px;">
        <label class="pub-toggle" for="bc-pub">
          <input type="checkbox" id="bc-pub" name="publish" value="1" <?= $bl["is_published"] ? "checked" : "" ?>>
          Publish immediately
        </label>
      </div>
      <button type="submit" class="btn-save-post">Save Changes <i class="fa-solid fa-check"></i></button>
    ...
    <div class="control-card">
      ...
      <?php if ($bl["image_path"]): ?>
        <div class="img-preview">
          <img loading="lazy" src="<?= htmlspecialchars($bl["image_path"]) ?>" alt="Preview">
          <span>Current image</span>
        </div>
      <?php endif; ?>

      <div class="file-upload-wrapper">
        <label for="bc-img" class="file-upload-btn"><i class="fa-solid fa-cloud-arrow-up"></i> Replace Image</label>
    ...
    <div class="control-card">
      ...
      <div id="seo-fields" style="display: none; margin-top: 15px; border-top: 1px dashed #e1e0eb; padding-top: 15px;">
        <div class="fg">
          ...
          <input type="text" class="scribe-input" id="bc-slug" name="slug" value="<?= htmlspecialchars($bl["slug"] ?? "") ?>" placeholder="e.g. creative-couple-prompts-guide">
          <span style="font-size:0.65rem; color:#8c8994; font-weight:600; display:block; margin-top:3px; margin-bottom:10px; line-height:1.3;">Edit the page URL. Lowercase letters, numbers, and hyphens only. Leave blank to keep current slug.</span>
        </div>
        <div class="fg">
          ...
          <input type="text" class="scribe-input" id="bc-mt" name="meta_title" value="<?= htmlspecialchars($bl["meta_title"] ?? "") ?>" placeholder="Leave blank to use post title">
        </div>
        <div class="fg">
          ...
          <textarea class="scribe-textarea" id="bc-md" name="meta_description" rows="3" placeholder="Short SEO description (150 chars recommended)"><?= htmlspecialchars($bl["meta_description"] ?? "") ?></textarea>
        </div>
        <div class="fg">
          ...
          <input type="text" class="scribe-input" id="bc-tags" name="tags" value="<?= htmlspecialchars($bl["tags"] ?? "") ?>" placeholder="AI, couple, prompts, creative (comma separated)">
        </div>
      </div>
    </div>
  </aside>
