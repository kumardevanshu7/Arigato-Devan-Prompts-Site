<?php require_once __DIR__ . '/includes/session_bootstrap.php'; ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta name="theme-color" content="#2F4156">
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Terms &amp; Conditions &ndash; Arigato Devan Prompts</title>
    <meta name="description" content="Terms &amp; Conditions for Arigato Devan — rules for using our AI couple prompts, unlock system, accounts, and content on arigatodevan.com.">
    <link rel="icon" href="/favicon.ico" type="image/x-icon">
    <meta property="og:type" content="website">
    <meta property="og:site_name" content="Arigato Devan Prompts">
    <meta property="og:title" content="Terms &amp; Conditions - Arigato Devan">
    <meta property="og:description" content="Rules for using our AI couple prompts, unlock system, user accounts, and content ownership on Arigato Devan.">
    <meta property="og:image" content="https://arigatodevan.com/landingpics/lan9.webp">
    <meta property="og:url" content="https://arigatodevan.com/terms.php">
    <meta name="twitter:card" content="summary_large_image">
    <meta name="robots" content="index, follow">
    <?php include_once 'includes/theme_head.php'; ?>
    <link rel="stylesheet" href="css/info-pages.css?v=20260727">
    <?php include_once "gtag.php"; ?>
</head>
<body class="page-store page-info theme-nogoda">

<?php $nav_active = ''; include 'includes/site_nav.php'; ?>
<div class="nogoda-mesh" aria-hidden="true"></div>

<main class="info-page-main">
    <div class="legal-card">
        <h1><i class="fa-solid fa-file-contract"></i> Terms &amp; Conditions</h1>
        <div class="legal-date">Effective Date: July 27, 2026 &nbsp;&middot;&nbsp; arigatodevan.com</div>

        <div class="legal-highlight">
            <i class="fa-solid fa-clipboard"></i> By visiting, browsing, or using Arigato Devan in any way, you agree to be bound by the Terms &amp; Conditions below. Please read them carefully before using the site.
        </div>

        <h2>1. Who We Are &amp; What This Site Does</h2>
        <p><strong>Arigato Devan</strong> ("we", "us", "our", "the Service") is a website at <strong>arigatodevan.com</strong> that publishes ready-to-copy AI prompts for creating couple-themed photos and reels using third-party AI image tools such as <strong>Google Gemini</strong> and <strong>ChatGPT</strong>. We provide a searchable gallery of prompts, a "Curated AI Prompts" section of carefully refined and enhanced prompts inspired by various creative sources, a Prompt of the Day feature, and a blog with prompting guides. These Terms &amp; Conditions ("Terms") govern your use of the Service, whether or not you create an account.</p>

        <h2>2. Acceptance of Terms</h2>
        <p>By accessing or using Arigato Devan, you confirm that you have read, understood, and agree to these Terms and our <a href="privacy.php">Privacy Policy</a>. If you do not agree with any part of these Terms, please stop using the Service immediately.</p>

        <h2>3. Eligibility &amp; Age Requirement</h2>
        <p>You must be at least <strong>13 years old</strong> to use Arigato Devan. If you are under the age of majority in your country, you confirm that you have permission from a parent or legal guardian to use the Service. We do not knowingly allow children under 13 to create accounts.</p>

        <h2>4. How the Service Works</h2>
        <ul>
            <li><strong>Browsing without an account:</strong> Anyone can view and copy prompts from the public gallery for free — no login is required to copy a prompt.</li>
            <li><strong>Accounts:</strong> Signing in with <strong>Google (Firebase Authentication)</strong> lets you save prompts permanently, unlock content with fewer taps, and comment on blog posts.</li>
            <li><strong>Unlock system:</strong> Some prompts require an interaction ("tap") count or an unlock code before the full prompt text is revealed. Unlock thresholds may differ for logged-in and logged-out visitors and may change at our discretion without prior notice.</li>
            <li><strong>Unlock codes:</strong> Codes are distributed only through our official channels (Instagram, on-site giveaways, community events). Selling, trading, or publicly redistributing unlock codes without our written permission is not allowed, and we may revoke access for accounts found misusing codes.</li>
        </ul>

        <h2>5. Licence to Use Prompts &amp; Content</h2>
        <p>Subject to these Terms, we grant you a limited, non-exclusive, non-transferable licence to use prompts obtained from Arigato Devan for <strong>personal and commercial purposes</strong>, including social media posts, reels, client work, and other creative projects. This licence is subject to the following conditions:</p>
        <ul>
            <li>You may use unlocked prompts to generate images or videos for your own use, including monetised content.</li>
            <li>You may <strong>not</strong> resell, redistribute, republish, or repackage our prompt text itself (as opposed to the images you generate from it) as a standalone product, course, or prompt pack without our prior written permission.</li>
            <li>You may <strong>not</strong> claim authorship or ownership of the original prompt wording, nor represent an unmodified copy of it as your own creative work.</li>
            <li>Images or videos you generate using our prompts belong to you, subject to the terms of service of whichever AI platform (Gemini, ChatGPT, or otherwise) you used to generate them.</li>
            <li>We may revoke this licence for any user found violating these conditions, without affecting content you have already lawfully created.</li>
        </ul>

        <h2>6. Third-Party AI Tools — No Affiliation, No Guarantee of Results</h2>
        <p>Arigato Devan is an independent prompt-writing resource. We are <strong>not affiliated with, endorsed by, or sponsored by</strong> Google, Gemini, OpenAI, ChatGPT, or any other AI provider mentioned on this site; all trademarks belong to their respective owners. We do not operate these AI tools and have no control over how they interpret a prompt. As a result:</p>
        <ul>
            <li>We cannot guarantee that any prompt will produce an identical, accurate, or "face-perfect" result every time — output quality depends on the AI model, your source photo, and the platform's own rules and limitations.</li>
            <li>You are solely responsible for complying with the terms of service, content policies, and acceptable-use rules of any third-party AI platform you use to run our prompts.</li>
            <li>You are responsible for the images you upload to any AI tool — only upload photos of yourself or people who have given you permission to use their likeness. Do not upload another person's photo without their consent.</li>
        </ul>

        <h2>7. User Accounts &amp; Security</h2>
        <ul>
            <li>Accounts are created using Google Sign-In; we do not store passwords.</li>
            <li>You are responsible for keeping your Google account secure. We are not liable for any loss arising from unauthorised access caused by a compromised Google account.</li>
            <li>One person should not operate multiple accounts to abuse the unlock system, referral features, or promotions.</li>
            <li>We reserve the right to suspend or permanently terminate any account that violates these Terms, misuses unlock codes, or engages in abusive behaviour toward other users or our team.</li>
        </ul>

        <h2>8. User-Generated Content (Comments, Feedback &amp; Reviews)</h2>
        <p>If you leave a blog comment, testimonial, or feedback submission, you agree that it will not contain hate speech, harassment, spam, illegal content, or content that infringes someone else's rights. You grant us a non-exclusive, royalty-free licence to display, edit for length/clarity, or remove such content on the Service. We may moderate, hide, or delete any submission at our discretion.</p>

        <h2>9. Intellectual Property Ownership</h2>
        <p>Unless otherwise credited (such as in the "Curated AI Prompts" section), all original prompt text, site design, graphics, logos, and written content on Arigato Devan are the intellectual property of Arigato Devan and are protected under applicable copyright and trademark laws. The "Curated AI Prompts" section includes prompts that have been refined and enhanced from various creative sources; where the original source is known, we aim to credit it — if you are a creator and want a prompt credited, amended, or removed, contact us using the details in Section 16.</p>

        <h2>10. Advertising</h2>
        <p>Arigato Devan may display advertisements served by <strong>Google AdSense</strong> and other advertising partners to support the free content on this site. These ads may use cookies to show relevant advertising, as described in full in our <a href="privacy.php">Privacy Policy</a>. We do not control the specific ad creative shown by these networks and are not responsible for the content of third-party advertisements.</p>

        <h2>11. Prohibited Conduct</h2>
        <p>While using Arigato Devan, you agree that you will not:</p>
        <ul>
            <li>Use the Service to create, request, share, or promote illegal, non-consensual, sexually explicit, hateful, or otherwise harmful content;</li>
            <li>Upload another person's photo to generate content without that person's consent;</li>
            <li>Attempt to scrape, reverse-engineer, copy, or clone the platform's code, database, or design;</li>
            <li>Circumvent, disable, or attempt to bypass our unlock system, security measures, or access controls;</li>
            <li>Use bots, scripts, or automated tools to interact with the Service or artificially inflate views, likes, or unlocks;</li>
            <li>Impersonate any person or entity, or misrepresent your affiliation with Arigato Devan.</li>
        </ul>

        <h2>12. Disclaimer of Warranties</h2>
        <p>Arigato Devan is provided on an <strong>"as is"</strong> and <strong>"as available"</strong> basis, without warranties of any kind, whether express or implied. We do not guarantee that the Service will be uninterrupted, error-free, or that any prompt will produce a specific result. We make reasonable efforts to keep the site accurate and up to date but cannot guarantee it will always be free of mistakes.</p>

        <h2>13. Limitation of Liability</h2>
        <p>To the maximum extent permitted by law, Arigato Devan and its creator shall not be liable for any indirect, incidental, special, or consequential damages arising from your use of the Service, including but not limited to loss of data, loss of content generated on third-party AI platforms, or issues arising from those third-party platforms' own terms or outages.</p>

        <h2>14. Changes to These Terms</h2>
        <p>We may update these Terms from time to time to reflect changes in the Service, legal requirements, or our business practices. The "Effective Date" above will always reflect the latest version. Continued use of Arigato Devan after changes are posted constitutes your acceptance of the revised Terms. We recommend checking this page periodically.</p>

        <h2>15. Governing Law</h2>
        <p>These Terms are governed by and construed in accordance with the laws of <strong>India</strong>, without regard to conflict-of-law principles. Any disputes arising from these Terms or your use of the Service shall be subject to the jurisdiction of the courts of India.</p>

        <h2>16. Contact Us</h2>
        <p>If you have questions about these Terms, need a "Curated AI Prompts" prompt credited or removed, or want to report misuse, reach us at:</p>
        <ul>
            <li>Email: <a href="mailto:devansh.grow@gmail.com">devansh.grow@gmail.com</a></li>
            <li>Contact form: <a href="contact.php">arigatodevan.com/contact.php</a></li>
            <li>Instagram: <a href="https://www.instagram.com/arigato.devan/" target="_blank" rel="noopener">@arigato.devan</a></li>
        </ul>

        <div style="text-align:center; margin-top:32px;">
            <a href="index.php" class="cta-btn cta-btn-ghost"><i class="fa-solid fa-arrow-left"></i> Back to Home</a>
        </div>
    </div>
</main>

<?php include 'footer.php'; ?>
</body>
</html>

