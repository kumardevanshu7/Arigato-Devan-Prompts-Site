...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'upload') {
    ...
    if (!$err && !nm_table_exists_admin($pdo, 'curated_prompts')) {
    ...
    if (!$category || !$title || !$prompt_text) {
        ...
        $thumb = curated_upload_image($_FILES['thumbnail'] ?? ['error' => UPLOAD_ERR_NO_FILE], 'thumb', 600, 800);
        if (!$thumb) {
            $err = 'Thumbnail image is required.';
        } else {
            $chatgpt_img = null;
            if (!$chatgpt_failed) {
                $chatgpt_img = curated_upload_image($_FILES['chatgpt_image'] ?? ['error' => UPLOAD_ERR_NO_FILE], 'cgpt', 900, 1600);
                if (!$chatgpt_img) {
            ...
            if (!$err && !$gemini_failed) {
                $gemini_img = curated_upload_image($_FILES['gemini_image'] ?? ['error' => UPLOAD_ERR_NO_FILE], 'gem', 900, 1600);
                if (!$gemini_img) {
...
<title>Curated AI Prompts — Admin</title>
...
<div class="main">
    <div class="page-header">
        <h1><i class="fa-solid fa-wand-magic-sparkles"></i> Curated AI Prompts — Upload</h1>
    ...
    <div class="share-box" id="uploadShareBox">
        ...
        <div class="share-row">
            ...
            <a href="curated_links.php" class="copy-link-btn" style="text-decoration:none"><i class="fa-solid fa-list"></i> All Links</a>
        </div>
    </div>
    <?php endif; ?>
