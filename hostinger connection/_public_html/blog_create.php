...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$title) {
        ...
        if (empty($error)) {
            ...
            $stmt = $pdo->prepare(
                "INSERT INTO blogs (title, slug, description, content, content_hindi, image_path, image_ratio, meta_title, meta_description, tags, is_published, author_id, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())"
            );
            $stmt->execute([
                $title,
                $slug,
...
<form method="POST" action="blog_create.php" enctype="multipart/form-data" id="blogForm" style="display:flex; flex-direction:column; height:100vh; overflow:hidden;">
