...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit_prompt') {
    ...
    if (!$image_helpers_ready) {
        ...
        $cur = $pdo->prepare('SELECT * FROM not_mine_prompts WHERE id = ?');
        $cur->execute([$edit_id]);
        ...
        if (!$row) {
            ...
            if (!$category || !$title || !$prompt_text) {
                ...
                $new_thumb = not_mine_upload_image($_FILES['thumbnail'] ?? ['error' => UPLOAD_ERR_NO_FILE], 'thumb', 600, 800);
                if ($new_thumb) {
                ...
                if ($chatgpt_failed) {
                    ...
                    $new_cgpt = not_mine_upload_image($_FILES['chatgpt_image'] ?? ['error' => UPLOAD_ERR_NO_FILE], 'cgpt', 900, 1600);
                    if ($new_cgpt) {
                        nm_unlink_image($chatgpt_img);
                ...
                if (!$err && $gemini_failed) {
                    ...
                    $new_gem = not_mine_upload_image($_FILES['gemini_image'] ?? ['error' => UPLOAD_ERR_NO_FILE], 'gem', 900, 1600);
                    if ($new_gem) {
                        nm_unlink_image($gemini_img);
                        $gemini_img = $new_gem;
                    }
                ...
                if (!$err) {
                    $slug = ($title !== $row['title'] || empty($row['slug']))
                        ? uniqueNotMineSlug($pdo, $title, $edit_id)
                        : $row['slug'];
                    $pdo->prepare(
                        'UPDATE not_mine_prompts SET category = ?, title = ?, slug = ?, tags = ?, prompt_text = ?, meta_description = ?, meta_keywords = ?,
                    ...
                    header('Location: not_mine_manage.php?edit=' . $edit_id . '&saved=1');
                    exit();
                }
            }
        }
...
$edit_row = null;
if ($edit_id > 0 && nm_table_exists_manage($pdo, 'not_mine_prompts')) {
    $es = $pdo->prepare('SELECT * FROM not_mine_prompts WHERE id = ?');
    $es->execute([$edit_id]);
...
$prompts = [];
if (nm_table_exists_manage($pdo, 'not_mine_prompts')) {
    try {
...
<title>Not Mine — Manage Prompts</title>
...
<aside class="sidebar">
  ...
  <a href="not_mine_manage.php" class="sb-link active"><i class="fa-solid fa-table-list"></i> <span>Manage Prompts</span></a>
  ...
  <div class="edit-card">
    ...
    <form method="POST" enctype="multipart/form-data" id="editForm">
      ...
      <div class="edit-actions">
        ...
        <a href="not_mine_manage.php" class="btn secondary">Cancel</a>
  ...
  <div class="list-card">
    ...
    <form method="GET" class="filters">
      ...
      <div class="filters-actions">
        ...
        <a href="not_mine_manage.php" class="btn secondary">Reset</a>
      </div>
    </form>
    ...
    <table class="list-table">
      ...
      <?php foreach ($filtered as $p): ?>
        <tr>
          ...
          <td>
            <a href="not_mine_manage.php?edit=<?= (int) $p['id'] ?>" class="btn-edit<?= $edit_id === (int) $p['id'] ? ' active' : '' ?>" title="Edit prompt"><i class="fa-solid fa-pen"></i></a>
